...
import shared.realmeditor.RealmEditorTestEvent;
import shared.realmeditor.editor.AutoMapSaver;
import shared.realmeditor.editor.MEBrush;
import shared.realmeditor.editor.MEClipboard;
...
import shared.realmeditor.editor.TimeControl;
import shared.realmeditor.editor.ToolSwitchEvent;
import shared.realmeditor.editor.actions.MapEditAction;
import shared.realmeditor.editor.tools.MESelectTool;
import shared.realmeditor.editor.tools.METool;
import shared.realmeditor.editor.ui.elements.MultiOptionalSwitch;
...
public class MainView extends Sprite {
    ...
    private var setScreen:SetScreenSignal;
    private var titleView:Sprite;

    public function MainView(titleView:Sprite, embedded:Boolean) {
        this.setScreen = StaticInjectorContext.getInjector().getInstance(SetScreenSignal);
        this.titleView = titleView;
        Instance = this;
        WebMain.STAGE.addEventListener(Event.RESIZE, this.onStageResize);
        StageWidth = WebMain.STAGE.stageWidth;
        StageHeight = WebMain.STAGE.stageHeight;
        ScaleX = WebMain.STAGE.stageWidth / 800;
        ScaleY = WebMain.STAGE.stageHeight / 600;
        ...
        this.window = WebMain.STAGE.nativeWindow;

        this.background = new Bitmap(new EmbeddedAssets.editorBg().bitmapData);
    ...
    private function addedToStage(e:Event):void {
        ...
        WebMain.STAGE.addEventListener(Event.ENTER_FRAME, this.update);
        WebMain.STAGE.addEventListener(MouseEvent.MOUSE_WHEEL, this.onMouseWheel);
        WebMain.STAGE.addEventListener(Event.RESIZE, this.onStageResize);
        WebMain.STAGE.addEventListener(Event.REMOVED_FROM_STAGE, this.onRemovedFromStage);
        Main = parent;
    ...
    private function onBackClick(e:Event):void {
        ...
        this.setScreen.dispatch(this.titleView);
    }
    ...
    private function getMouseTilePosition():IntPoint { // Not to handle null value
        if (this == null || this.mapView == null) {
            return null;
        }
